...
items = ["贵金属","铜(CU)","螺纹(RB)","热卷(HC)","铝(AL)","锌(ZN)","铅(PB)","镍(NI)","不锈钢(SS)","铁矿石(I)",
         ...
         "生猪","花生","白糖","鸡蛋","苹果","豆粕","棕榈油","豆油","大豆","股指","宏观","铝","铅","锌","螺纹","热卷","铁矿石","工业硅"]
next = False
prev_item = ""
for l in lines:
...
with open('国信.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
items = ["股指","国债","贵金属","铜铝","玻璃","锌镍","锌","纯碱","螺纹钢","工业硅","锰硅：","硅铁：","铁合金","焦炭焦煤","镍","豆类","油脂","白糖","棉花","玉米","生猪","花生","苹果","PTA","聚烯烃","原油","橡胶","燃料油","沥青","甲醇","铁矿石","动力煤"]
next = False
prev_item = ""
for l in lines:
    stripped = l.strip().strip('\n').strip('【').strip('】')
...
with open('东兴.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
items = ["期指","期债","动力煤","铜","PTA","TA","PVC","钢矿","天然橡胶","生猪","玉米","橡胶","MA","煤焦","油脂","棉花","化工","螺纹"]
next = False
prev_item = ""
for l in lines:
    stripped = l.strip()
...
with open('一德.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
items = ["期指","期债","黄金/白银","螺纹/热卷","螺纹/热卷(RB2310/HC2310)","螺纹/热卷(RB2305/HC2305)","煤焦","郑糖","塑料/PP","塑料/pp","硅锰","硅铁","动力煤","铁矿石","沪铝","沪镍","沪铜","沪锌",
         "沪铅","苹果","红枣","鸡蛋","生猪","外糖","燃料油","白糖","硅","甲醇","PVC","PTA","MEG","纯碱","玻璃","塑料/PP","苯乙烯","聚酯","原油","工业硅","沪锡","锰硅/硅铁"]
...
for l in lines:
    if is_number(l.strip('\n').split(" ")[-1]):
        ...
        company = l.strip('\n').strip("#")
order = ["","小结：","金融","股指","股指期权","国债","黑色金属","螺纹钢","纯碱玻璃","铁矿","焦煤","焦炭","螺纹","热卷","硅铁","锰硅","工业硅","有色金属","氧化铝","铝","铜","锌","锡","镍","不锈钢","铅","贵金属","黄金","白银","能源化工",
