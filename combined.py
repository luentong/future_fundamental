...
###########################################国投安信开始
with open('国投安信.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
prev = ""
for l in lines:
...
with open('光大.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
prev = ""
topop = []
...
items = ["贵金属","铜(CU)","螺纹(RB)","热卷(HC)","铝(AL)","锌(ZN)","铅(PB)","镍(NI)","不锈钢(SS)","铁矿石(I)",
         ...
         "铜及国际铜","镍及不锈钢","PTA及短纤","天然橡胶","棉花及棉纱","玉米及玉米淀粉","原油","沥青","聚丙烯","塑料","PVC","甲醇","苯乙烯","纯碱","尿素","焦煤","焦炭",
         "生猪","花生","白糖","鸡蛋","苹果","豆粕","集运指数（欧线）","碳酸锂","铝及氧化铝","棕榈油","豆油","大豆","股指","宏观","铝","铅","锌","螺纹","热卷","铁矿石","工业硅","生猪及鸡蛋","玉米"]
next = False
prev_item = ""
for l in lines:
...
with open('五矿.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
items = ["股指","国债","油脂","碳酸锂","蛋白粕","焦炭：","焦煤：","焦煤焦炭","高、低硫燃料油","鸡蛋","生猪","白糖","苹果","棉花","贵金属","铜","锌","铅","原油",
...
with open('广发.txt') as f:
    lines = f.readlines()
idea = {}
items = ["国债期货：","贵金属：","铜：","锌：","铝：","镍：","不锈钢：","锡：","碳酸锂：","钢材：","铁矿石：","焦炭：","焦煤：","动力煤：","豆粕：","油脂：","生猪：","玉米：","白糖：","棉花：","鸡蛋：","花生：","红枣：",
         "原油：","沥青：","PTA：","乙二醇：","燃料油：","LPG：","短纤：","苯乙烯：","LLDPE：","PP：","尿素:","PVC：","甲醇：","纯碱：","玻璃：","玻璃:","纯碱:","橡胶：","纸浆：","苹果：","工业硅","工业硅："]
next = False
prev_item = ""
for l in lines:
    stripped = l.strip().strip('\n').strip('【').strip('】')
...
with open('国信.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
items = ["股指","国债","贵金属","铜铝","碳酸锂","纯碱：","尿素","焦炭焦煤","玻璃","铜铝、氧化铝","锌镍","锌","纯碱","铜","螺纹钢","工业硅","铝 氧化铝","锰硅：","硅铁：","铁合金","焦炭焦煤","镍","豆类","油脂","白糖","棉花","玉米","生猪","花生","苹果","PTA","聚烯烃","原油","橡胶","燃料油","沥青","甲醇","铁矿石","动力煤"]
next = False
prev_item = ""
for l in lines:
    stripped = l.strip().strip('\n').strip('【').strip('】')
...
with open('南华.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
items = ["国债日报","股指","螺纹","热卷","碳酸锂","铁矿","焦煤","焦炭","锰硅","尿素","硅锰","硅铁","动力煤","纯碱","玻璃","白糖","棉花","苹果","红枣","油料","油脂","原油","油脂油料","甲醇","燃料油","PVC","聚酯","沥青",
         "PTA","MEG","PF","TA","EG","LPG","鸡蛋","镍&不锈钢","铝&氧化铝","纸浆","橡胶","玉米&淀粉","工业硅","铜","铝","锌","镍不锈钢","锡","豆一","贵金属","聚烯烃","螺纹钢","铁矿石","郑棉","豆粕","生猪","MA","SC","RU","EB","乙二醇","苯乙烯","PTA&PF","硅锰观点"]
next = False
prev_item = ""
for l in lines:
    stripped = l.strip()
...
with open('东兴.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
items = ["期指","期债","动力煤","铜","PTA","甲醇","TA","PVC","钢矿","天然橡胶","生猪","玉米","橡胶","MA","煤焦","油脂","棉花","化工","螺纹"]
next = False
prev_item = ""
for l in lines:
    stripped = l.strip()
...
with open('混沌天成农产品.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
items = ["油脂油料","棉花","玉  米","豆  粕","鸡  蛋","生  猪","苹  果","纸  浆","红  枣","白  糖"]
next = False
prev_item = ""
for l in lines:
    stripped = l.strip().strip('\n').strip('【').strip('】')
...
items = ["原油","PTA","乙二醇","短纤","聚烯烃","液化石油气","铁合金","沥青","甲醇","苯乙烯","橡胶","玻璃","纯碱","尿素","纸浆","黄金&白银","沪镍","沪铜&国际铜","沪铝","沪锌","沪铅",
         "螺纹&热卷","铁矿石","焦煤&焦炭","碳酸锂","动力煤","聚乙烯","聚丙烯","钢材","PVC","油脂","豆一","油料","工业硅","花生","玉米&淀粉","棉花&棉纱","生猪","鸡蛋","白糖","苹果","红枣","国债","股指"]
next = False
prev_item = ""
for l in lines:
    stripped = l.strip().strip('\n').strip('【').strip('】')
...
items = ["螺卷","铁矿","双焦","双硅","原油","沥青","LPG","甲醇","PVC","天然橡胶",
         ...
         "生猪","苹果","红枣","花生","MEG","铜","铝","锌","铅","工业硅","棉花棉纱","玻璃纯碱"]
...
with open('一德.txt', encoding='gbk') as f:
    lines = f.readlines()
idea = {}
items = ["期指","期债","黄金/白银","贵金属","螺纹/热卷","螺纹/热卷(RB2310/HC2310)","螺纹/热卷(RB2305/HC2305)","煤焦","郑糖","塑料/PP","塑料/pp","硅锰","硅铁","动力煤","铁矿石","沪铝","沪镍","沪铜","沪锌",
         "沪铅","苹果","红枣","鸡蛋","碳酸锂","生猪","外糖","燃料油","锌","白糖","硅","甲醇","PVC","PTA","MEG","纯碱","玻璃","塑料/PP","苯乙烯","聚酯","原油","工业硅","沪锡","锰硅/硅铁"]
...
for i in [guangzhou_old, hualian_old, dongxing_old, guotai_old, yongan_old, zhongxin_old,
          ...
          hundungong_old, hundunneng_old, hundunagri_old, guodu_old, zhongtai_old
          ]:
    for j in i:
...
for l in lines:
    if is_number(l.strip('\n').split(" ")[-1]):
        ...
        company = l.strip('\n').strip("#")
order = ["","小结：","金融","股指","股指期权","国债","黑色金属","螺纹钢","纯碱玻璃","铁矿","焦煤","焦炭","螺纹","热卷","硅铁","锰硅","工业硅","有色金属","碳酸锂","氧化铝","铝","铜","锌","锡","镍","不锈钢","铅","贵金属","黄金","白银","能源化工",
 "原油","燃油","低硫燃油","LPG","沥青","甲醇","乙二醇","MEG","PTA","短纤","苯乙烯","PVC","PP","塑料","尿素","橡胶","合成橡胶","纯碱","玻璃","纸浆","农产品","棕榈油","豆油","菜油","豆粕","豆一","玉米","淀粉","鸡蛋","白糖",'糖',"棉花",
 "苹果","花生","红枣","生猪","集运","集运指数（欧线）","纯碱&玻璃"]
...
for i in [guangzhou_idea, hualian_idea, dongxing_idea, guotai_idea, yongan_idea, citrix_idea,
          ...
          hundungong_idea, hundunneng_idea, hundunagri_idea, guodu_idea, zhongtai_idea
          ]:
    for j in i:
        if j.strip() == "":
            abc = 1
